...
def _get_teacher_id_from_session():
    """Return teacher id from session_state.teacher_data.

    Your Supabase `teachers` table primary key might be stored under different
    column names (e.g. `teacher_id`, `id`, etc.).
    """
    td = st.session_state.get("teacher_data") or {}
    for key in ("teacher_id", "id", "teacherId"):
...
def teacher_tab_take_attendance():
    teacher_id = _get_teacher_id_from_session()
    if teacher_id is None:
        st.error(
            "Teacher id missing. Ensure your Supabase `teachers` table has a PK column (e.g. `teacher_id` or `id`) and that `teacher_login()` returns it."
        )
        return
